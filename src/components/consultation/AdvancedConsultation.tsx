...
export default function AdvancedConsultation() {
  ...
  const nextStep = () => {
    // Basic validation
    if (currentStep === 0) {
      if (!formData.fullName || !formData.email || !formData.phone || !formData.dateOfBirth || !formData.gender || !formData.age || !formData.address) {
        setError("Please fill all fields in this step");
        return;
      }
    }
    if (currentStep === 1) {
      if (!formData.symptoms || !formData.medications || !formData.allergies || !formData.chronicCondition) {
        setError("Please fill all fields in this step");
        return;
      }
    }
    if (currentStep === 2) {
      if (!formData.consultationDate || !formData.consultationTime || !formData.consultationType || !formData.emergencyContactName || !formData.emergencyContactNumber) {
        setError("Please fill all fields in this step");
        return;
      }
    }
    ...
    <div className="bg-white">
            ...
            {doctors.length > 0 ? doctors.map((doc, index) => (
              <motion.div key={index} whileHover={{ y: -12 }} className="group bg-white rounded-3xl shadow-xl overflow-hidden transition duration-300">
                ...
                <div className="p-8">
                  ...
                  <button onClick={() => setSelectedDoctor(doc)} className="mt-6 px-6 py-2 bg-black text-white rounded-full hover:bg-[var(--color-primary)] transition cursor-pointer">
                    View Profile
                  </button>
                </div>
              </motion.div>
            ...
            <div className="bg-white rounded-3xl shadow-xl max-w-2xl w-full p-8 relative overflow-y-auto max-h-[90vh]">
               ...
               <div className="flex flex-col md:flex-row gap-8 items-center md:items-start mb-8">
                  <div className="w-32 h-32 relative rounded-full overflow-hidden border-4 border-gray-100 shrink-0">
                     <Image src={selectedDoctor.image} alt={selectedDoctor.name} fill className="object-cover" unoptimized/>
                  </div>
      ...
      <section className="relative py-20 bg-[var(--color-beige)] overflow-hidden">
        <h2 className="text-4xl md:text-5xl font-bold text-[var(--color-dark)] leading-tight mb-8 pl-150">
            Consultation
            <span className="block text-[var(--color-primary)] mt-2 pl-20">Form</span>
        </h2>
          ...
          <div className="bg-white/70 backdrop-blur-xl border border-white/30 rounded-3xl shadow-2xl p-16">
            ...
            <form onSubmit={handleSubmit}>
              <motion.div key={currentStep} initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.4 }}>
                ...
                {currentStep === 0 && (
                  <div className="space-y-6">
                    <div className="grid md:grid-cols-2 gap-6">
                      <input type="text" name="fullName" value={formData.fullName} onChange={handleChange} required placeholder="Full Name" className="form-input" />
                      <input type="email" name="email" value={formData.email} onChange={handleChange} required placeholder="Email Address" className="form-input" />
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                      <input type="tel" name="phone" value={formData.phone} onChange={handleChange} required placeholder="Phone Number" className="form-input" />
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      ...
                      <input type="number" name="age" value={formData.age} onChange={handleChange} required placeholder="Age" className="form-input" />
                    </div>
                    <input type="text" name="address" value={formData.address} onChange={handleChange} required placeholder="Full Address" className="form-input" />
                  </div>
                )}
                ...
                {currentStep === 1 && (
                  <div className="space-y-6">
                    <textarea name="symptoms" value={formData.symptoms} onChange={handleChange} required rows={4} placeholder="Describe your current symptoms" className="form-input rounded-3xl" />
                    <textarea name="medications" value={formData.medications} onChange={handleChange} required rows={3} placeholder="Current medications (if any)" className="form-input rounded-3xl" />
                    <textarea name="allergies" value={formData.allergies} onChange={handleChange} required rows={3} placeholder="Known allergies" className="form-input rounded-3xl" />
                    <select name="chronicCondition" value={formData.chronicCondition} onChange={handleChange} required className="form-input">
                ...
                {currentStep === 2 && (
                  <div className="space-y-6">
                    <div className="grid md:grid-cols-2 gap-6">
                      <input type="date" name="consultationDate" value={formData.consultationDate} onChange={handleChange} required className="form-input" />
                      <input type="time" name="consultationTime" value={formData.consultationTime} onChange={handleChange} required className="form-input" />
                    </div>
                    <select name="consultationType" value={formData.consultationType} onChange={handleChange} required className="form-input">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <input type="text" name="emergencyContactName" value={formData.emergencyContactName} onChange={handleChange} required placeholder="Emergency Contact Name" className="form-input" />
                      <input type="tel" name="emergencyContactNumber" value={formData.emergencyContactNumber} onChange={handleChange} required placeholder="Emergency Contact Number" className="form-input" />
                    </div>
                  </div>
                )}
                ...
                {currentStep === 3 && (
                  <div className="space-y-8">
                    ...
                    <button type="submit" disabled={loading} className="w-full bg-black text-white py-4 rounded-full hover:bg-[var(--color-primary)] transition text-lg cursor-pointer disabled:opacity-50">
                      {loading ? "Booking..." : "Submit Consultation Request"}
                    </button>
                  </div>
                )}
